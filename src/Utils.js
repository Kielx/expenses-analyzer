...
export const parseDataForGraphUsageIncome = (parsed) => {
  let temp = [];
  if (parsed) {
    temp = parsed.map((data) => {
      if (
...
export const prepareExpensesData = (parsedFile) => {
    ...
    parsedFile.forEach((item) => {
      if (item["#Tytuł"]) {
        ...
        if (checked < 0) {
          ...
          temp[checked]["amount"] += parseFloat(
            item["#Kwota"].replace(" ", "")
          );
        }
      }
